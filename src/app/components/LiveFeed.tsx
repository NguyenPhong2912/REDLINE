  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={panel()}>
      <div className="flex items-center gap-3 px-4 py-3 border-b" style={{ borderColor: color.border, background: color.surfaceSubtle }}>
        ...
        <span className="text-[11px]" style={{ ...mono, color: color.textMuted }}>runtime · {status === "live" ? "live feed" : status}</span>
      ...
      <div className="p-4 space-y-2 min-h-[180px]" style={{ background: term.bg }}>
        {rows.length === 0 && <div className="text-[11px]" style={{ ...mono, color: term.dim }}>{status === "offline" ? "Start the backend (cd backend && npm run dev) to see live events." : "Waiting for events — create a grant and start an agent."}</div>}
        {rows.map(e => {
          const d = describe(e);
          return (
