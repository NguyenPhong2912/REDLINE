...
function describe(e: FeedEvent): { color: string; text: string } {
  ...
  switch (e.eventType) {
    ...
    case "run.ended": return { color: "#64748b", text: `agent runtime ended · ${p.reason}` };
    ...
    case "decision.precheck": return p.allow ? { color: "#64748b", text: `precheck · all gates passed` } : { color: A, text: `precheck DENY · ${p.reasonCode} — ${p.message}` };
    ...
    default: return { color: "#64748b", text: e.eventType };
  }
}
...
export function LiveFeed({ grantId = "*", limit = 12 }: { grantId?: string; limit?: number }) {
  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{ background: "rgba(11,17,16,0.6)", border: "1px solid rgba(255,255,255,0.05)", boxShadow: "0 8px 40px rgba(0,0,0,0.4)" }}>
      <div className="flex items-center gap-3 px-4 py-3 border-b" style={{ borderColor: "rgba(255,255,255,0.05)", background: "rgba(1,3,3,0.5)" }}>
        ...
        <span className="text-[11px]" style={{ ...mono, color: "#94a3b8" }}>runtime · {status === "live" ? "live feed" : status}</span>
        <div className="ml-auto flex items-center gap-1.5">
          ...
          <span className="text-[10px] font-bold" style={{ ...mono, color: status === "live" ? M : status === "offline" ? R : A }}>{status === "live" ? "LIVE" : status === "offline" ? "OFFLINE" : "…"}</span>
        </div>
      </div>
      <div className="p-4 space-y-2 min-h-[180px]" style={{ background: "#010303" }}>
        {rows.length === 0 && <div className="text-[11px]" style={{ ...mono, color: "#334155" }}>{status === "offline" ? "Start the backend (cd backend && npm run dev) to see live events." : "Waiting for events — create a grant and start an agent."}</div>}
        {rows.map(e => {
          const d = describe(e);
          return (
            <div key={e.id} className="flex gap-3 text-[11px]" style={mono}>
              <span style={{ color: "rgba(148,163,184,0.3)", minWidth: 60 }}>{new Date(e.at).toLocaleTimeString("en-US", { hour12: false })}</span>
