...
- Wallet sessions cover ownership, but the shared API key still opens the write routes that do not check an owner, and it ships in the frontend bundle where anyone can read it. Retiring it entirely means giving every write route an owner to check against. No professional audit has been performed; mainnet use is out of scope.
